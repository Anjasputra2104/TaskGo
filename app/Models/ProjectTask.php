    ...
    public static function getAllSectionedTaskList($request, $project, $filterdata = [], $not_task_ids = [])
    {
        ...
        if(!empty($task_ids) && count($task_ids) > 0)
        {
            ...
            foreach($task_ids as $task_id)
            {
                ...
                $taskCollectionArray['taskinfo'] = json_decode(app('App\Models\Http\Controllers\ProjectTaskController')->getDefaultTaskInfo($request, $task->id), true);
        ...
        if(!empty($section_ids) && count($section_ids) > 0)
        {
            foreach($taskSections as $section_id => $section_name)
            {
                ...
                {
                    $sectiontasks[$onekey]['taskinfo'] = json_decode(app('App\Models\Http\Controllers\ProjectTaskController')->getDefaultTaskInfo($request, $onetask['id']), true);
                }
